...
int main(int argc, char * argv[]) {
  ...
  size_t block_size = 128; /* 1kB -- CHANGE THIS! */
  size_t mem_size = 1024 * 1024 * block_size; /* 1MB -- CHANGE THIS! */
  int character;

  character = getopt(argc, argv, "b:s:");
  while (character != -1) {
	  switch (character) {
	  case 'b':
...
void ackerman_main(size_t _block_size, size_t _mem_size) {
  ...
  */
  ...
  for (;;) { /* Loop forewer, or until we break. */
    ...
    cout << "The parameters for _block_size and _mem_size for MyAllocator are: " << _block_size << " ; " << _mem_size << endl;
